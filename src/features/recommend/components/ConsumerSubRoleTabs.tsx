import type { RecommendSubRole } from '../types';

export function ConsumerSubRoleTabs({
  value,
  onChange,
}: {
  value: RecommendSubRole;
  onChange: (next: RecommendSubRole) => void;
}) {
  const options: Array<{ key: RecommendSubRole; label: string; desc: string }> = [
    { key: 'business', label: '🎯 业务运营', desc: '更偏增长与可落地动作' },
